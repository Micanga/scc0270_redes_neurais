...
v1, v2, v3, v4, v5 = [], [], [], [], []
with open('Aula2-exec2.csv') as inputfile:
	for line in inputfile:
		if re.match("^\d+.\d+,\d+.\d+,\d+.\d+,\d+.\d+,\d+$",line) is not None:
			data = line.split(',')
			v1.append(float(data[0]))
			v2.append(float(data[1]))
			v3.append(float(data[2]))
			v4.append(float(data[3]))
			v5.append(int(data[4]))
			number_of_examples += 1

# 2. Initializing the Neuron
neuron = Neuron(nweights=4)
...
for bias in np.arange(-5,5,0.5):
	...
	for w1 in np.arange(-5,5,0.5):
		for w2 in np.arange(-5,5,0.5):
			for w3 in np.arange(-5,5,0.5):
				for w4 in np.arange(-5,5,0.5):
					...
					for i in range(number_of_examples):
						x, y = np.array([v1[i],v2[i],v3[i],v4[i]]),v5[i]

						# d. defining neuron parameters
						neuron.weight = np.array([w1,w2,w3,w4])
						neuron.bias = bias
					...
					threshold_result.append([bias,w1,w2,w3,w4,threshold_error])
					linear_result.append([bias,w1,w2,w3,w4,linear_error])
					sigmoid_result.append([bias,w1,w2,w3,w4,sigmoid_error])
